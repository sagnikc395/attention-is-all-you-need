...
def build_token_to_id_vocab(sentences, specials=('<pad>', '<bos>', '<eos>', '<unk>')):
    ...
    # first , assigning the IDs to special tokens in the order they are given 
    for token in specials:
        if token not in vocabulary:
            vocabulary[token] = len(vocabulary)

    # then. process the corpus to add new unique tokens in first seen order 
...
def encode_sentence_to_ids(sentence, token_to_id, unk_token='<unk>'):
    ...
    # sentence by whitespace and look up each token's Id.abs
    # falling back to unk_id if the token not in the vocabulary 
